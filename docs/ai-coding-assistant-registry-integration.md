...
> (CLI on macOS, Linux, Windows) and [Claude.ai Custom Connectors](https://support.anthropic.com/en/articles/11175166-getting-started-with-custom-connectors-using-remote-mcp).
> Microsoft Entra ID support via the Client ID Metadata Document
> (CIMD, `draft-parecki-oauth-client-id-metadata-document`) is the next
> milestone; see [Roadmap](#roadmap) below.
>
...
- **Kiro CLI**: same story; supports static `Authorization: Bearer ...` headers.
